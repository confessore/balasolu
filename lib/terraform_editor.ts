...
export const getCleanFile = () => {
    const data = fs.readFileSync(
        'terraform/mongo-dev/main.template.tf', { encoding: 'utf8'})
    return data
}

const instances = 250

export const buildMongoFiles =
    (resourceName: string, resourcePort: number) => {
...
export const initMongoFiles =
    () => {
        for (var i = 0; i < instances; i++)
        {
            ...
            exec(`terraform -chdir=${path} init`,
...
export const applyMongoFiles =
    () => {
        for (var i = 0; i < instances; i++)
        {
            const path = `terraform/mongo-dev/main${i}/`
            var exec = require('child_process').exec;
            exec(`terraform -chdir=${path} apply -auto-approve`,
            function (error: string | null, stdout: string, stderr: string) {
                console.log('stdout: ' + stdout);
                console.log('stderr: ' + stderr);
                if (error !== null) {
...
export const destroyMongoFiles =
    () => {
        for (var i = 0; i < instances; i++)
        {
            const path = `terraform/mongo-dev/main${i}/`
            var exec = require('child_process').exec;
            exec(`terraform -chdir=${path} destroy -auto-approve`,
            function (error: string | null, stdout: string, stderr: string) {
                console.log('stdout: ' + stdout);
                console.log('stderr: ' + stderr);
                if (error !== null) {
